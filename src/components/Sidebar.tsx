  ...
  const handleSpecialOfferToggle = (offerType: string) => {
    if (selectedSpecialOffer === offerType) {
      setSelectedSpecialOffer(""); 
       ...
       if (offerType.includes('Hot')) {
        setSelectedSpecialOffer("HotDeals");
       } else if (offerType.includes('Mega')) {
        setSelectedSpecialOffer("MegaDiscount");
       } else if (offerType.includes('Flash')) {
        setSelectedSpecialOffer("FlashSales");
       }
    }
  };
...
const offerCounts = filteredProducts.reduce(
  (acc, product) => {
...
const categoryCounts = filteredProducts.reduce(
 ...
 const specialOffers: SpecialOfferOption[] = [
  { type: "Hot Deals", count: offerCounts["HotDeals"] || 0 },
  { type: "Mega Discount", count: offerCounts["MegaDiscount"] || 0 },
  { type: "Flash Sales", count: offerCounts["FlashSales"] || 0 },
];
...
const categoryOptions: CategoryOption[] = Object.entries(categoryCounts).map(
  ...
  return (
    <aside className="w-1/3 min-h-screen bg-white p-4 space-y-6">
          ...
          <div className="mt-4 space-y-3">
            {padToSix(specialOffers, { type: "Placeholder", count: 0 }).map(
              (deal, i) => (
                <div
                  key={i}
                  className={`flex justify-between items-center text-sm cursor-pointer transition-colors ${
        ...
        {openSections.prices && (
          <div className="mt-4">
            <p className="text-sm text-gray-700 mb-4">
              Range: ${selectedPrice[0]} – ${selectedPrice[1]}
            </p>
      ...
      <div className="bg-[#f6f7f8] rounded-md p-4 text-center">
